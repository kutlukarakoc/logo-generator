...
export function LogoCard({ logo, showPrompt = true }: LogoCardProps) {
  ...
  const downloadLogo = async () => {
    try {
      ...
      if (status !== 'granted') {
        Alert.alert('Permission required', 'Please allow access to your media library to download the logo');
        return;
      }
      ...
      // Download the image
      const downloadResult = await FileSystem.downloadAsync(logo.imageUrl, fileUri);

      if (downloadResult.status === 200) {
        // Save to media library
        const asset = await MediaLibrary.createAssetAsync(downloadResult.uri);
        await MediaLibrary.createAlbumAsync('Logos', asset, false);
